...
export function DataSourceBanner({ meta, onRetry, retrying }: DataSourceBannerProps) {
  ...
  const serverUp = meta.serverApiOnline === true;
  const frontendUp = meta.frontendOnline === true;

  if (meta.source === 'live' && serverUp) {
    return (
      <div className="mb-4 rounded-lg border border-neon-green/30 bg-neon-green/5 px-4 py-2 text-xs text-neon-green">
        ...
        Server/API verbunden
        {frontendUp ? ' · Frontend erreichbar' : ' · Frontend nicht erreichbar'}
      </div>
    );
  }
      ...
      <div className="mb-4 rounded-lg border border-neon-red/40 bg-neon-red/10 px-4 py-3 text-sm text-red-100">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="min-w-0 flex-1">
            <p className="font-semibold text-neon-red">Haupt-App Server/API nicht erreichbar</p>
            <p className="mt-1 text-xs text-red-200/90">{banner}</p>
            {meta.lastError ?
              <p className="mt-2 font-mono text-[11px] text-red-200/70">
                Technisch: {meta.lastError}
              </p>
            : null}
          </div>
          {onRetry ?
  ...
  const body =
    !meta.apiConfigured ?
      'Bitte RABBITSTATION_API_URL und CONTROL_CENTER_API_TOKEN setzen.'
    : meta.message ??
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          ...
          <ul className="mt-3 space-y-1 font-mono text-[11px] text-red-200/80">
            <li>RABBITSTATION_CLIENT_URL: {meta.clientUrlDisplay ?? (meta.apiUrlSet ? 'Fallback API-URL' : 'fehlt')}</li>
